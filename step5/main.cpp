...
int main (int argc, char **argv)
{
    ...
    // 記号音階
    std::vector<std::string> treble_notes = {"C4", "D4", "E4", "F4", "G4", "A4", "B4", "C5", "D5", "E5", "F5", "G5", "A5", "B5"};
    std::vector<std::string> bass_notes = {"E2UU", "F2UU", "G2UU", "A2UU", "B2", "C3", "D3", "E3", "F3", "G3", "A3", "B3", "C4", "D4"};
    // 音符を保存する
    std::vector<std::string> unit_notes;

    std::vector<double> scan_x(5, 20);
    std::vector<double> scan_y(5, 0);
    std::vector<int> y_targets = {4, 4, 4, 3, 3, 2, 2, 1, 1, 0, 0, 0, 0, 0};
    std::vector<double> width_coef = {1, 0.5, 0, 0.5, 0, 0.5, 0, 0.5, 0, 0.5, 0, -0.5, -1, -1.5};
    std::vector<std::string> found_note;
    int systems = argc > 1 ? 2 : 1;
    ...
    while (capture.isOpened()) {
      ...
      // 高さで検出点をソートする
      sort(lines.begin(),lines.end(),[](const cv::Vec2f &alpha, const cv::Vec2f &beta){return alpha[0] < beta[0];});
      ...
      for(; it!=lines.end(); ++it) {
        ...
        if(std::hypot(x0-past_x0, y0-past_y0) >= 6 && abs(theta - CV_PI / 2) < 0.1) {
          // 認識した五線の描画
      ...
      if (systems == 1) {
        ...
        line_is_scanned = std::none_of(line_y.begin(), line_y.end(), [](int x) {return x == 0;});
        line_is_five = abs((line_y[0] + line_y[4]) / 2 - line_y[2]) <= 20
                          && abs((line_y[5] + line_y[9]) / 2 - line_y[7]) <= 20;
      }
      if (line_is_scanned && line_is_five) {
        ...
        for (int system = 0; system < systems; ++system) {
          ...
          for (int i = 0; i < 5; ++i) {
            auto j = i + system * 5;
            if (i != 0) {
              scan_x[i] = (line_y[i+1] - line_y[i]);
            }
            scan_y[i] = line_y[j]+(scan_x[0]-line_x[j])*tan(line_th[j]-M_PI / 2);
          }
          // 五線譜の幅を概算
          double width = 0;
          for (int i = 0; i < 4; ++i) {
          ...
          // スキャニングバーの描画
          cv::line(dst_img, cv::Point(scan_x[0], scan_y[0]-width*2), cv::Point(scan_x[0], scan_y[4]+width*2), cv::Scalar(255,204,51), 3, cv::LINE_AA);
          // 音符の認識
          int range_x = width / 5;
          int range_y = width / 4 - 1; 
          int note_index = 0;
          static double found_x = -100;
          if (abs(found_x - scan_x[0]) >= 2) {
            found_note = {};
            for (auto note : (system == 0 ? treble_notes : bass_notes)) {
              int sum = 0;
              for (int i = -range_y; i <= range_y; i++) {
                for (int j = -range_x; j <= range_x; j++) {
                  ...
                  if (note_index >= 3 && note_index <= 9 && note_index % 2 == 1) {
                    ...
                    sum += static_cast<int>(dst_bin_img.at<unsigned char>(cv::Point(scan_x[0] + j, 
                      center + i)));
                  } else {
                    sum += static_cast<int>(dst_bin_img.at<unsigned char>(cv::Point(scan_x[0] + j, 
                      scan_y[y_targets[note_index]] + i + width*width_coef[note_index])));
                  }
                }
              }
              if (sum == 0) {
            ...
            // 認識した音符の確認
            if (found_note.size() != 0) {
              std::cout << (system == 0 ? "treble note : " : "bass note : ");
            }
        ...
        scan_x[0] <= 600 ? scan_x[0]+=3+(4-4) : scan_x[0] = 20;
      }
